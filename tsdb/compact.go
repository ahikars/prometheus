...
func (b *blockBaseSeriesSet) Next() bool {
	...
	for b.p.Next() {
		...
		b.currLabels = lbls
		b.currIter = newPopulateWithTombBaseSeries(b.chunks, chks, intervals)
	}
	return false
}
...
func newPopulateWithTombBaseSeries(
	...
	var bufIter *deletedIterator
	if len(intervals) > 0 {
		bufIter = &deletedIterator{intervals: intervals}
	}
...
func (p *populateWithTombBaseSeriesIterator) next() bool {
	for p.err == nil && p.i < len(p.chks)-1 {
		...
		if len(p.bufIter.intervals) == 0 || !p.currChkMeta.OverlapsClosedInterval(
		...
		p.bufIter.it = p.currChkMeta.Chunk.Iterator(p.bufIter.it)
...
func (p *populateWithTombBaseSeriesIterator) toSeriesIterator() chunkenc.Iterator {
	return &populateWithTombSeriesIterator{populateWithTombBaseSeriesIterator: p}
}
func (p *populateWithTombBaseSeriesIterator) toChunkSeriesIterator() chunks.Iterator {
	return &populateWithTombChunkSeriesIterator{populateWithTombBaseSeriesIterator: p}
}
